...
namespace Eze_Basket
{
    ...
    public partial class home : ContentPage
    {
        ...
        private async void startbackgrounding()
        {
            ...
            for (int h = 0; h < 10; h++)
            {
                if (reader.Read())
                {
                    ...
                    Task<StackLayout> task5 = new Task<StackLayout>(fifthcall);
                    task5.Start();
                    StackLayout ee = await task5;
                    fisrtstack.Children.Add(ee);

                }
            }
             
        }
        StackLayout fifthcall()
        {
            ...
            StackLayout btneditor = new StackLayout() { Spacing = 0, Padding = 0, Margin = 0, BackgroundColor = Color.LightBlue };
            StackLayout vb = new StackLayout() { Spacing = 0, Padding = 0, Margin = 0,BackgroundColor=Color.LightBlue };
            Frame bnm = new Frame() { Margin = 0, Padding = 0,CornerRadius=5,BorderColor=Color.LightGray };
            ed[framecount] = new Editor() { HorizontalOptions = LayoutOptions.Fill, HeightRequest = 60,WidthRequest=200 };
            ed[framecount].StyleId = framecount.ToString();
            bnm.Content = ed[framecount];
            Button br = new Button(); br.Text = "Add comment"; br.StyleId = framecount.ToString(); br.Clicked += new System.EventHandler(this.buttonclickbr);
            vb.Children.Add(dr[framecount]);
            btneditor.Children.Add(bnm);btneditor.Children.Add(br);
            vb.Children.Add(dr[framecount]); vb.Children.Add(btneditor);
             framecount++;
            return vb;
        }
        ...
        bool firstcall()
        {
            ...
            MySqlCommand cmd = new MySqlCommand("SELECT * FROM product_table ORDER BY RAND()", con1);
            con1.Open();
      ...
      //  {
       ...
       // }
        ...
        private void buttonclickj2(object sender, EventArgs e)
        {
            ...
            rer = cc.ExecuteReader(); rer.Read();
            Label m = new Label(); m.Text ="SPECIFICATIONS: "+rer["specification"].ToString();//specification
            f[asdf].Content = null; f[asdf].Content = m;
            co.Close();
        }
